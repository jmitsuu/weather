...
<template>
  <div class="flex flex-col  justify-center h-full ">
    <h1 class="font-sans text-4xl font-bold text-gray-200 text-center">Previsão do Tempo</h1>
    ...
    <div class="mt-9 w-full p-6 rounded-md  " :class="setBgWeather" v-if="temperature">
      <div class="flex flex-col bg-black bg-opacity-30 p-2 rounded-lg" >
       ...
       <div class="flex justify-center items-center ">
        <p class=" text-gray-200 text-xl text-center "> {{ item.weather[0].description }} </p>
          <img  class="h-20" :src="`https://openweathermap.org/img/wn/${item.weather[0].icon}.png`" />
       </div>
          <div v-for="item in temperature" :key="item.temperature">
            <div class="text-center mb-2">
              <p class=" text-gray-200 text-4xl ">{{ item.main.temp.toFixed(0) }}C°</p>
            

            </div>
            <!-- <img class="m-auto rounded-lg w-10" :src="item.weather_icons[0]" /> -->
            <div class="flex bg-slate-200 bg-opacity-80 p-4 rounded-lg   text-center mt-4 justify-center gap-9 ">
              <div>
                <p class="text-gray-600 text-xl">Sensação Térmica</p>
                <P class="text-slate-900 border-2 p-2 rounded-xl font-bold ">{{ item.main.feels_like.toFixed(0) }}C°</P>
              </div>
              <div class="">
                <p class="text-gray-600 text-xl">Max/Min</p>
               <div class="flex border-2  p-2 rounded-xl  font-bold">
                <P class="text-slate-900 ">{{ item.main.temp_max.toFixed(0) }}C°  -</P>
                <P class="text-slate-900 ml-1"> {{ item.main.temp_min.toFixed(0) }}C° </P>
               </div>
              </div>
              <div>
                <p class="text-gray-600 text-xl">Humidade</p>
                <P class="text-slate-900 border-2 rounded-xl  p-2 font-bold">{{ item.main.humidity }}%</P>
              </div>
        
              <div>
                <p class="text-gray-600 text-xl">Ventos</p>
                <P class="text-slate-900 border-2  rounded-xl  p-2 font-bold">{{ item.wind.speed.toFixed(0) }} Km/h</P>
              </div>
...
async function getWeather() {
  const {data} = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${searchCity.value}&appid=43744dd7f1d88e520ee89b5e41fd554b&units=metric&lang=pt_BR`)
  weatherCity.value = [data]
